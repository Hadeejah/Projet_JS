        ...
        if (mnt.value < 500) {
            ...
            if (inputNum.value == "") {
                ...
                if (+mnt.value > +solde.textContent) {
                    ...
                    if (getUserTel(inputNum.value) == undefined) {
                        ...
                        users[idPos].transaction.push(tra)
                        users[idPos].solde = +solde.textContent - (+mnt.value);
                        solde.textContent = users[idPos].solde
                        ...
                        traDep.numero = numTrans
                        traDep.sens = 'depot'
                        traDep.montant = mnt.value
                        traDep.DaTe = date

                        let k = getUserTel(inputNum.value)
                        numTrans++
                        users[k].transaction.push(traDep)
                        users[k].solde += +mnt.value
...
btnAnnuler.addEventListener('click', () => {
    ...
    let tra = {
        numero: 1,
        sens: 'depot',
        montant: tabVide[0].montant,
        DaTe: date
    }
    users[idPos].solde += +tra.montant
    users[idPos].transaction.push(tra)
    createTransaction(users[idPos].transaction)
    solde.innerText = +solde.innerText+tra.montant
    ...
    users[tabVide[0].id].solde -= +traRetrait.montant
    users[tabVide[0].id].transaction.push(traRetrait)
...
function transfert(user1, user2, rising, i, j) {
    user1[i].transaction.push({ numero: 1, DaTe: date, sens: 'depot', montant: +mnt.value, })
    console.log(user1[i].transaction);
    user1[i].solde += +rising
    user2[j].transaction.push({ numero: 1, DaTe: date, sens: 'retrait', montant: +mnt.value, })
...
function getUserTel(tel) {
    for (let k = 0; k < users.length; k++) {
        if (users[k].num_tel == tel) {
            return k
        }
